 # Lab 5: Excessive Agency (OWASP LLM06)
  ...
  ## Objective
  Demonstrate OWASP LLM06 (Excessive Agency) by building an unsafe
  agent that can be manipulated via prompt injection, then building
  a safe agent with least privilege, input validation, and audit
  logging.

  ## Background

  Excessive Agency occurs when an LLM agent is granted more
  permissions than necessary. If manipulated via prompt injection
  or malicious input, the agent can take destructive or unauthorized
