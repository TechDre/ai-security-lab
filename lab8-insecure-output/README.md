...
[Objective](#objective) | [Background](#background) | [Setup](#setup) | [Phase 1](#phase-1--xss-attack-via-llm-output) | [Phase 2](#phase-2--sql-injection-via-llm-output) | [Phase 3](#phase-3--command-injection-via-llm-output) | [Comparison](#comparison-unsafe-vs-safe) | [Security Observations](#security-observations) | [Key Takeaways](#key-takeaways) | [OWASP LLM02](#owasp-llm02-insecure-output-handling) | [Connections](#connection-to-other-labs) | [Defensive Recommendations](#defensive-recommendations)

## Objective

Demonstrate insecure output handling by showing how LLM output passed directly to downstream systems (browser, database, shell) can deliver XSS, SQL injection, and command injection attacks. Then implement output sanitization defenses for each attack type.

## Background

When LLM output is passed to another system without sanitization, the LLM becomes a delivery mechanism for classic injection attacks. The attacker does not need to hack the LLM itself — they just need to craft input knowing where the output will flow.
...
In a real browser, the script tag would execute JavaScript, stealing session cookies and sending them to an attacker.
...
**Unsafe result:**
```
[SQL QUERY]: SELECT * FROM users WHERE name = '' OR '1'='1'; DROP TABLE users; --'
[!] SQL INJECTION ATTACK - Query manipulated by LLM output!
```

This query would return every user in the database AND drop the entire users table.
...
**Safe result:**
```
[SQL QUERY - SAFE]: SELECT * FROM users WHERE name = ' OR 1=1 TABLE users '
[+] SQL injection characters removed!
```

> **Note:** In production always use parameterized queries instead of string sanitization — they are more reliable.
...
The semicolon chained a second command, same attack vector as demonstrated in Lab 5 (Excessive Agency).
...
- The LLM is the delivery mechanism, not the attacker — attackers craft input knowing where output will flow
- Every downstream system needs its own sanitization — HTML, SQL, and shell each have different dangerous characters
- Never interpolate LLM output directly into queries, commands, or rendered HTML
- Parameterized queries > string sanitization for SQL — Sanitization can be bypassed, but parameterization cannot
- Output handling is an application responsibility — the LLM will not protect you from this

## OWASP LLM02: Insecure Output Handling

This lab demonstrates the output handling dimension of OWASP LLM02:
- LLM output is untrusted data — treat it like user input to any other system
- The attack surface expands with every downstream system the LLM feeds into
...
- Treat LLM output as untrusted — sanitize before passing to any downstream system
- Use parameterized queries — never interpolate LLM output into SQL strings
- HTML escape all output rendered in browsers
- Apply shell metachar filtering before any exec call
- Combine with Lab 5 defenses — allowlisting + output sanitization for agent tool use
- Audit log all sanitization events — blocked payloads reveal active attack patterns
