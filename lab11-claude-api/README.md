[README.md](https://github.com/user-attachments/files/26576579/README.md)
...
## Objective
Build a working Claude API integration using the Anthropic SDK.
Demonstrate single message requests, multi-turn conversations
with system prompts, and structured output for security analysis.
...
A: Prompt injection is an attack where a user inserts malicious
instructions into an input to manipulate an AI model into ignoring
its original guidelines and performing unintended actions. It
exploits the model's tendency to follow all text it receives,
potentially bypassing safety measures or causing it to behave
in harmful ways.
...
Maintain conversation context across multiple turns with a
...
Claude maintained context across all 3 turns without re-explaining
previous answers — demonstrating stateful conversation.
...
EXPLANATION: This is a direct attempt to override instructions
and extract system information through social engineering.
```
...
Haiku is the most cost-efficient Claude model — ideal for labs
and development.
...
2. **Set max_tokens** — prevents runaway costs from large responses
3. **Validate API responses** before using in downstream systems
4. **Treat Claude output as untrusted** — apply Lab 8 output
   sanitization before rendering or executing
5. **Monitor token usage** — unexpected spikes indicate misuse
